...
Photo.statics.upload = function(req){
  return new Promise((resolve, reject) => {
    if(!req.file){
      return reject(new Error('Multi-part Form Data Error. File not present on request.'));
    }
    if(!req.file.path){
      return reject(new Error('Multi-part Form Data Error. File path not present on request.'));
    }
