...
const Photo = mongoose.Schema({
  ...
  iamgeUrl: { type: String, required: true, unique: true }
})
...
Photo.static.upload = function (params) {
  return new Promist((resolve, reject) => {
    ...
    return awsS3.uploadProm(params) //ships all the data to s3
      .then(data => {
        //del([`${tempDir}/*`]) //sotres temporary binary data. del is short for fs.link. Kind of works as a middleware to check on what data has been processed

        let photoData = {
          name: request.body.name,
          description: request.body.description,
          userId: request.user._id,
          galleryId: reqest.body.galleryId,
